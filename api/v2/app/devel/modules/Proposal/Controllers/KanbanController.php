...
class KanbanController extends \Micro\Controller {
            ...
            ->filter(function($status) use ($params) {
                ...
                if ($task) {
                    ...
                    if (isset($params['tts_id'])) {
                        // load document to
                        $document = Proposal::findFirst($task->tt_document);
    ...
    public function updateAction($id) {
        ...
        if (isset($post['worker'], $post['record'], $post['record']['task'])) {
            ...
            if ($task->save($form['task'])) {
                ...
                if (isset($form['document'])) {
                    $document = Proposal::findFirst($form['task']['tt_document']);
                    if ($document) {
                        $form['document']['status'] = $form['task']['tt_flag'];
    ...
    public function deleteAction($id) {
        ...
        if ($task) {
            ...
            if ($document) {
                if ($document->status == 'BELUM DI PROSES') {
                    $document->delete();
                    $remove = TRUE;
                } else {
